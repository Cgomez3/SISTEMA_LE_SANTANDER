 ...
 * @author EXPERTYA
 */
public class ProgramacionPagos extends javax.swing.JInternalFrame {
    ...
    public ProgramacionPagos() {
        initComponents();
        this.setTitle("Programación de Pagos");
    ...
    private void listacomprobante() {
        ...
        if (liscom.size() > 0) {
            for (int i = 0; i < liscom.size(); i++) {
                ...
                if(liscom.get(i).getDestinoPago()!=null){
                ...
                if(liscom.get(i).getComprobanteRef()!=null){
                    nuevafila[28] = liscom.get(i).getComprobanteRef();    
                }else{
                    nuevafila[28] = "";        
                }
                if(liscom.get(i).getRazonSocialRef()!=null){
                    nuevafila[29] = liscom.get(i).getRazonSocialRef();    
                }else{
                    nuevafila[29]="";
                }
                if(liscom.get(i).getFechaRef()!=null){
    ...
    private boolean validardetalle(String banco, String moneda){
        proveedoressincuenta="<html><body width='500'><h4>Los siguientes proveedores no tienen cuenta bancaria: </h4>";
        LProgPagosCabecera lp = new LProgPagosCabecera();
        ArrayList<String> lstprov1=new ArrayList<>();
        ArrayList<String> lstprov2=new ArrayList<>();
        ...
        for (int i = 0; i < liscom2.size(); i++) {
            if (lp.validarctabancaria(banco, moneda, liscom2.get(i).getDocProveedor()) == 1) {
                ...
                System.out.println(liscom2.get(i).getRazonSocial());
                ...
                proveedoressincuenta=proveedoressincuenta+"<p>-"+liscom2.get(i).getRazonSocial()+"<br>"+ "  ";
            }
        }
    ...
    private void initComponents() {
        ...
        lbltitulo.setText("PROGRAMACION DE PAGOS");
        ...
        jPanel6.add(cbocuentaprincipal, new org.netbeans.lib.awtextra.AbsoluteConstraints(350, 163, 110, -1));
        ...
        jPanel6.add(jLabel3, new org.netbeans.lib.awtextra.AbsoluteConstraints(30, 150, 450, 38));
    ...
    private void btnguardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnguardarActionPerformed
        ...
        if (validarcabecera()) {
            ...
            int fila = 0;
            tipestado = (tblistacomprobante.getValueAt(fila, 4)).toString().trim();
            ...
            if (validardetalle(objtc.getBanco(), objtc.getMoneda())) {
                if (lstdetalleprog.size() == liscom2.size()) {
                    ...
                    if (JOptionPane.showConfirmDialog(this, "Se va GRABAR la Programación de Pagos. ¿Desea Continuar?", "Confirmar", JOptionPane.YES_NO_OPTION) == (JOptionPane.YES_OPTION)) {                        
                         if (ltc.InsertarPrgPagos(objtc)) {
                            ...
                            limpiar();
                            JOptionPane.showMessageDialog(this, "Se grabó correctamente la Programación N° "+codigomax+".");
                        }
                    }
                    }
                } else {
                    if (JOptionPane.showConfirmDialog(null, proveedoressincuenta.substring(0, proveedoressincuenta.length()-2)+"Grabar sólo los comprobantes(proveedores) que tienen cuenta bancaria?", "Confirmar", JOptionPane.YES_NO_OPTION) == (JOptionPane.YES_OPTION)) {
                        if (ltc.InsertarPrgPagos(objtc)) {
                            MaxCodigo();
                            for (int i = 0; i < lstdetalleprog.size(); i++) {
                                ltc.insertarProgDetalle(lstdetalleprog.get(i), codigomax);
                ...
                JOptionPane.showMessageDialog(this, "Ninguno de los comprobantes seleccionados tiene cuenta bancaria para realizar el pago.");
            }
        }
...
//            llenartabitems();
        ...
        if (filas.length > 1) {
            for (int i = 0; i < filas.length; i++) {
                ...
                for (int j = 0; j < liscom2.size(); j++) {
                    if (liscom2.get(j).getCodComprobante() == Integer.parseInt(tblistacomprobante.getValueAt(filas[i], 0).toString())) {
                        rep = 1;
                    }
                }
                if (rep == 0) {
                    EComprobante atencion = new EComprobante();
                        atencion.setCodComprobante(Integer.parseInt(tblistacomprobante.getValueAt(filas[i], 0).toString()));
                        atencion.setTipoComprobante(tblistacomprobante.getValueAt(filas[i], 1).toString());
                        atencion.setNumComprobante(tblistacomprobante.getValueAt(filas[i], 2).toString());
                        atencion.setFechaComprobante(tblistacomprobante.getValueAt(filas[i], 3).toString());
                        atencion.setEstado(tblistacomprobante.getValueAt(filas[i], 4).toString());
                        atencion.setRazonSocial(tblistacomprobante.getValueAt(filas[i], 5).toString());
                        atencion.setDocProveedor(tblistacomprobante.getValueAt(filas[i], 6).toString());
                        atencion.setGlosaComprante(tblistacomprobante.getValueAt(filas[i], 7).toString());
                        atencion.setComprobanteSujeto(tblistacomprobante.getValueAt(filas[i], 8).toString());
                        atencion.setTipoDetra(tblistacomprobante.getValueAt(filas[i], 9).toString());
                        atencion.setFormaPago(tblistacomprobante.getValueAt(filas[i], 10).toString());
                        atencion.setDestinoPago(tblistacomprobante.getValueAt(filas[i], 11).toString());
                        atencion.setNomMoneda(tblistacomprobante.getValueAt(filas[i], 12).toString());
                        ...
                        atencion.setComprobanteRef(tblistacomprobante.getValueAt(filas[i], 28).toString());
                        atencion.setRazonSocialRef(tblistacomprobante.getValueAt(filas[i], 29).toString());
                        atencion.setFechaRef(tblistacomprobante.getValueAt(filas[i], 30).toString());
    ...
    private javax.swing.JComboBox cbobancoprincipal;
    private javax.swing.JComboBox cbocuentaprincipal;
